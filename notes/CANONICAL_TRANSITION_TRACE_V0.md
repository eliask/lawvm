...
# LawVM Canonical Transition Trace v0

This spec defines the artifact family a certificate checker REPLAYS: the
base-tree representation, the content-blob store, the canonical transition
row grammar with full action semantics, and the per-date state-root
(checkpoint) semantics. It is the companion to
[CERTIFICATE_SCHEMA_V0.md](CERTIFICATE_SCHEMA_V0.md): the certificate commits
to `base_tree_root`, `canonical_transition_root`, `content_blobs_root`, and
...
The trace is the L3 layer of the three-level operation model:

```text
L1  source ops (amendment language)        — live only inside the compiler
L2  resolved legal operations              — engine-produced; carried on
    ("at address A, effective D, replace     transitions for DISPLAY only
    /insert/repeal payload P")
L3  certified tree transitions             — "at address A with subtree hash
...
## 5. Transition rows (`trace/canonical_transitions.jsonl`)
...
```text
transition leaf = LeafHash("lawvm.canonical_transition.v0", certified core)
canonical_transition_root =
  ListRoot("lawvm.canonical_transition_trace.v0",
