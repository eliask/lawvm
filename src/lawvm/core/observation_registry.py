...
FINDING_REGISTRY: Dict[str, FindingSpec] = {f.code: f for f in (
    ...
    FindingSpec("PARSE.VTS_SOURCE_UNREADABLE_OR_EMPTY", "frontend_extraction",
                "source_pathology", "warn", "vts",
    ...
    FindingSpec("PARSE.VTS_PARAGRAPHIZED_REPEAL_FRAGMENT_UNPARSED", "frontend_extraction",
                "source_pathology", "warn", "vts",
    ...
    FindingSpec("TEMPORAL.DURATION_ARITHMETIC_AUTHORITY_MISSING", "fixed_term_expiry",
                "ambiguity", "strict_fail", "fixed_term_expiry",
                "whole-law validity is a duration from commencement; the end date must not be computed without the pinned 150/1930 arithmetic rule",
                ("temporal_selection", "strictness"), role="obligation"),
    FindingSpec("TEMPORAL.EVENT_BOUND_RESOLVER_MISSING", "fixed_term_expiry",
                "ambiguity", "strict_fail", "fixed_term_expiry",
                "whole-law validity ends at a säädöskokoelma-discernible event (another instrument's entry into force); the cross-document resolver is not yet implemented",
                ("temporal_selection", "strictness"), role="obligation"),
    FindingSpec("TEMPORAL.EVENT_BOUND_OUT_OF_DOCTRINE", "fixed_term_expiry",
                "ambiguity", "strict_fail", "fixed_term_expiry",
                "whole-law validity ends at a substantive event not discernible from the säädöskokoelma; outside the blessed event-bound drafting pattern, never guessed",
                ("temporal_selection", "strictness"), role="obligation"),
    FindingSpec("TEMPORAL.SOURCE_IMPOSSIBLE_DATE", "fixed_term_expiry",
                "ambiguity", "strict_fail", "fixed_term_expiry",
                "source states a calendar-impossible validity end date; a candidate normalization is recorded but never used without a säädöskokoelma correction or manual attestation",
                ("temporal_selection", "strictness"), role="obligation"),
    ...
    FindingSpec("PARSE.JOHTOLAUSE_FAILED.RESOLVED_BY_ATTESTATION", "evidence_kernel",
                "recovery", "strict_fail", "evidence_kernel",
                "johtolause parse failure resolved via provenance attestation",
                ("parse_witness", "strictness"), role="barrier"),
