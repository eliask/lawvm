...
The cross-jurisdiction parity audit (XP-06) found EE was the only frontend with
ZERO ``WriteReceipt(...)`` construction sites: every other frontend (NO/SE/EU/UK;
FI reference) emits a per-op write receipt, EE did not. This module closes that
carrier gap entirely within ``src/lawvm/estonia/``.

Grounding-neutrality (the §2.7 byte-stable-bench invariant): receipts are
additive evidence. The EE replay path (``apply_ee_ops``) produces them ONLY when
a caller opts in by passing a ``write_receipts_out`` collection sink (mirroring
the existing ``lo_ops_out`` / ``adjudications_out`` / ``seam_observations_out``
side channels). With the sink absent (the default), the EE apply lane is
byte-identical to its pre-instrumentation behaviour — no snapshot is taken, no
diff is run. The receipt is the producer-side record; the independent observed
before/after diff (``core.observed_write_audit``) is the load-bearing
cross-check.

A RENUMBER op's bound→landed divergence (source label → destination label) is
the typed named migration ``ee_section_renumber_relabel``, mirroring SE's
``se_renumber_relabel``, NO's ``no_section_renumber_relabel`` and UK's
``uk_section_renumber_relabel``. Without that named rule the RENUMBER receipt is
a §1.6 unstated-migration violation strict mode must reject.
"""
...
#: Named migration rule id owning the bound→landed divergence on an EE RENUMBER
#: op (source label → destination label). Mirrors SE's ``se_renumber_relabel``,
#: NO's ``no_section_renumber_relabel`` and UK's ``uk_section_renumber_relabel``
#: (one id per family). Without it the divergence audits as a ``violation``;
#: with it, the receipt's :meth:`WriteReceipt.divergence_explained` is True.
EE_SECTION_RENUMBER_RELABEL_RULE_ID = "ee_section_renumber_relabel"


def _ee_action_name(op: LegalOperation) -> str:
    """Return the op's action as a plain string (enum or str tolerant).
...
def _ee_legal_path_to_tree_path(addr: LegalAddress) -> TreePath:
    ...
    Mirrors ``uk_write_receipts._uk_legal_path_to_tree_path``.
    """
...
def _resolve_with_recursive_fallback(body: IRNode, path: TreePath) -> IRNode | None:
    ...
    EE PEG ops emit flat paths (e.g. ``section:29``) even when the statute has
    chapters, so a single coordinate from ``op.target`` may not resolve by a
    strict top-down walk. Mirrors NO/UK's single-segment ``tree_ops.find``
    fallback (and EE's own ``_ee_resolve_full_path`` recursion in the grafter).
    """
...
def emit_ee_op_receipt(
    ...
    change); no receipt is emitted — the adjudication carries the witness.

    The mutation footprint is categorized by the op's action:
    REPLACE/text-replace → ``replaced_paths``; INSERT → ``created_paths``;
    REPEAL → ``removed_paths``; RENUMBER → ``renumbered_paths`` ``(from, to)``
    sourced from ``op.target`` and ``op.destination`` with
    ``migration_rule_ids=(ee_section_renumber_relabel,)``.
    ...
    from CERTIFIED_TREE_TRANSITION_TRACE_V0.md §2.2). For REPEAL the post hash is
    ``""`` (the hash of an absent subtree).
    """
    changed = diff_ir_paths_identity_pruned(before_body, after_body)
    if not changed:
        # Op filtered/skipped or no-op — the replay adjudication ledger carries
        # the witness instead; no receipt for a no-op apply.
        return None

    action_value = _ee_action_name(op)
    leaf_kind = op.target.leaf_kind() or "unknown"
    helper = f"apply_ee_ops::{action_value}::{leaf_kind}"
    bound_target_path: TreePath = _ee_legal_path_to_tree_path(op.target)
    ...
    # text_replace the targeted legal address IS the landed address (bound ==
    # landed semantically; the deep ``changed[0]`` is a tree-nesting artifact, not
    # a semantic divergence — EE sections live nested under chapters, like NO).
    # RENUMBER lands at the destination — bound != landed by construction,
    # explained by the named migration rule. Mirrors NO/UK action-keyed
    # landed/footprint resolution.
    if action_value in {"replace", "text_replace"}:
        ...
        destination_path = (
            _ee_legal_path_to_tree_path(op.destination)
        ...
        if destination_path is not None:
            ...
            migration_rule_ids = (EE_SECTION_RENUMBER_RELABEL_RULE_ID,)
    else:
