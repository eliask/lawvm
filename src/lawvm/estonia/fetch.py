...
def open_rt_archive_immutable(db_path: Optional[Path] = None) -> Any:
    ...
    Thin wrapper around :func:`lawvm.tools._worker_pool.open_farchive_immutable`
    that fills in the EE default path when ``db_path`` is omitted.  See that
    function's docstring for the full WSL2 / ``immutable=1`` rationale.

    Use ONLY for read-only consumers such as parallel bench workers — never for
    acquisition paths that write to the archive.
    """
    from lawvm.tools._worker_pool import open_farchive_immutable

    path = Path(db_path) if db_path is not None else _DEFAULT_RT_DB
    return open_farchive_immutable(path)
