...
from lawvm.finland.lowering_scope_recovery import (
    ...
    resolve_group_surface_scope as _resolve_group_surface_scope_impl,
...
from lawvm.finland.standalone_targets import (
    ...
    group_shadow_pruning_foreign_scoped_section_targets as _group_shadow_pruning_foreign_scoped_section_targets_impl,
    group_shadow_pruning_section_targets as _group_shadow_pruning_section_targets_impl,
)
...
def _group_has_scope_source(group_ops: Iterable[AmendmentOp], source: str) -> bool:
    """Backward-compat wrapper for lowering_scope_recovery.group_has_scope_source."""
    return _group_has_scope_source_impl(group_ops, source)


def _allow_unscoped_live_section_retarget(
    group_ops: Iterable[AmendmentOp],
) -> str | None:
    """Backward-compat wrapper for lowering_scope_recovery allowlist logic."""
    return _allow_unscoped_live_section_retarget_impl(group_ops)
...
def _source_body_chapter_for_scoped_section_target(
    *,
    ...
    """Backward-compat wrapper for lowering_scope_recovery source body lookup."""
    return _source_body_chapter_for_scoped_section_target_impl(
        muutos_tree=muutos_tree,
        target_norm=target_norm,
        target_chapter=target_chapter,
        target_part=target_part,
...
def _source_body_scope_for_section_target(
    *,
    ...
    """Backward-compat wrapper for lowering_scope_recovery source body scope."""
    return _source_body_scope_for_section_target_impl(
        muutos_tree=muutos_tree,
        target_norm=target_norm,
    )
...
def _resolve_group_surface_scope(
    *,
    ...
    """Backward-compat wrapper for lowering_scope_recovery surface scope."""
    return _resolve_group_surface_scope_impl(
        muutos_tree=muutos_tree,
        target_unit_kind=target_unit_kind,
        target_norm=target_norm,
        target_chapter=target_chapter,
...
def _group_shadow_pruning_section_targets(
    ...
    *,
    ...
    """Backward-compat wrapper for standalone_targets.group_shadow_pruning_section_targets."""
    return _group_shadow_pruning_section_targets_impl(
        ops,
        target_unit_kind=target_unit_kind,
        target_norm=target_norm,
        target_part=target_part,
        duplicate_section_labels=duplicate_section_labels,
    )
...
def _group_shadow_pruning_foreign_scoped_section_targets(
    ...
    *,
    ...
    """Backward-compat wrapper for standalone_targets foreign-scope selector."""
    return _group_shadow_pruning_foreign_scoped_section_targets_impl(
        ops,
        target_unit_kind=target_unit_kind,
        target_norm=target_norm,
        target_part=target_part,
...
def _stabilize_same_parent_relabel_order(resolved: List[ResolvedOp]) -> List[ResolvedOp]:
    """Backward-compat wrapper for relabel_identity.stabilize_same_parent_relabel_order."""
    return _stabilize_same_parent_relabel_order_impl(resolved)


def _stabilize_chapter_relabel_order(resolved: List[ResolvedOp]) -> List[ResolvedOp]:
    """Backward-compat wrapper for relabel_identity.stabilize_chapter_relabel_order."""
    return _stabilize_chapter_relabel_order_impl(resolved)


def _build_standalone_section_targets(
    ops: list[AmendmentOp],
) -> frozenset[StandaloneSectionTarget]:
    """Backward-compat wrapper for standalone_targets.build_standalone_section_targets."""
    return _build_standalone_section_targets_impl(ops)
