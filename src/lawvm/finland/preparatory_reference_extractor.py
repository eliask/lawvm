...
# EU act form + act number embedded in a paragraph.
# Two citation forms in Finnish Finlex:
#   Modern (EU post-2014): "(EU) YEAR/SEQUENTIAL"  e.g. "(EU) 2017/2226", "(EU) 2019/904"
#     → eu_year = 4-digit year, eu_number = sequential (1-6 digits)
#   Old (EY/ETY pre-EU): "(EY) N:o NUMBER/YEAR"    e.g. "(EY) N:o 178/2002"
#     → eu_year = 4-digit year (second), eu_number = sequential (1-6 digits, first)
# We use two sub-patterns in alternation, modern first:
#   modern: 4-digit/1-6-digit  →  eu_year = first, eu_number = second
#   old:    N:o 1-6-digit/4-digit  →  eu_year = second, eu_number = first
# Named groups differ per variant; caller extracts via eu_year/eu_n/eu_n_old.
# Substring guard: any of "(EU)", "(EY)", "(EEY)", "(ETY)"
_EU_ACT_MODERN_RE = re.compile(
    r'\((?P<form>EU|EY|EEY|ETY)\)\s+(?P<eu_year>\d{4})/(?P<eu_n>\d{1,6})\b'
)
# Old "N:o" form: (EY) N:o NUMBER/YEAR
_EU_ACT_NNUM_RE = re.compile(
    r'\((?P<form>EU|EY|EEY|ETY)\)\s+N:o\s+(?P<eu_n>\d{1,6})/(?P<eu_year>\d{4})\b'
)

# CELEX number: e.g. "32017R2226", "32009L0028"
# Type character: R=Regulation, L=Directive, D=Decision, etc.
# Substring guard: digit start + 4-digit year + letter + 4-digit number
_CELEX_RE = re.compile(
    r'\b(?P<celex>3(?P<cy>\d{4})(?P<ctype>[A-Z])(?P<cn>\d{4}))\b'
)

# OJ reference: "EUVL L 327, 9.12.2017, s. 20"
# Also handles older form "EYVL L 31, 1.2.2002, s. 1" and "EYVL N:o L 327..."
# Substring guards: "EUVL " or "EYVL "
_OJ_RE = re.compile(
...
class PreparatoryRefRecognizer:
    ...
    def _recognize_eu_paragraph(
        ...
        # Extract EU act fields — try modern format first, then N:o old format.
        # Modern: (EU) YEAR/SEQUENTIAL  e.g. "(EU) 2017/2226"
        # Old:    (EY) N:o NUMBER/YEAR  e.g. "(EY) N:o 178/2002"
        m_eu = _EU_ACT_MODERN_RE.search(text)
        if m_eu is None:
            m_eu = _EU_ACT_NNUM_RE.search(text)
        eu_form = m_eu.group("form") if m_eu else None
        eu_year = int(m_eu.group("eu_year")) if m_eu else None
        eu_number = int(m_eu.group("eu_n")) if m_eu else None

        # Extract CELEX if present
        m_celex = _CELEX_RE.search(text)
        celex: Optional[str] = m_celex.group("celex") if m_celex else None
        celex_type: Optional[str] = m_celex.group("ctype") if m_celex else None
        ...
        # Extract OJ fields
        m_oj = _OJ_RE.search(text)
        oj_series: Optional[str] = None
        oj_number_val: Optional[int] = None
        oj_date: Optional[date] = None
        oj_page: Optional[int] = None
        if m_oj:
            oj_series = m_oj.group("series")
            oj_number_val = int(m_oj.group("n"))
            oj_date = _parse_oj_date(m_oj.group("d"))
            oj_page = int(m_oj.group("p"))
