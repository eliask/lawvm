...
# head's inflected surfaces are generated by MORPHOLOGY (M1 paradigm inversion)
# rather than a hand-written ``sopimu(ksen|...)`` suffix enumeration, killing the
# consonant-gradation substring bug class.
#
# The case set is DELIBERATELY CURATED, not the full paradigm: a treaty governs
# an article in the cases that can head "<sopimus> N artiklassa" — singular
# NOM/GEN/PART/INE/ELA/ILL and plural NOM/GEN — exactly the set the original
# hand enumeration (``ksen|kseen|ksessa|ksesta|kset|s|sta|sten``) encoded.  The
# adessive/allative/ablative/translative ("by/onto/from/into this agreement",
# ``sopimuksella`` …) do NOT govern an article cite and are intentionally
# excluded: widening to the full paradigm regressed by emitting STATUTE_ONLY
# treaty references on adverbial ``sopimus`` forms that sit near an unrelated
# (often EU-asetus) article number.  ``head_case_forms`` generates exactly the
# curated ``(case, number)`` set so the gradated stem forms are sound M1 outputs,
# not substrings.  The ``yleis`` compound modifier (``yleissopimuksen``) and any
# other modifier stem rides invariant in front, as a statute modifier rides
# before ``laki``.
_SOPIMUS_HEAD_CASE_NUMBERS: tuple[tuple[str, str], ...] = (
    ("NOM", "SG"),
    ("GEN", "SG"),
    ("PART", "SG"),
    ("INE", "SG"),
...
_SOPIMUS_HEAD_FORMS: tuple[str, ...] = head_case_forms(
    "sopimus", _SOPIMUS_HEAD_CASE_NUMBERS
)
_SOPIMUS_HEAD_ALT = "|".join(re.escape(f) for f in _SOPIMUS_HEAD_FORMS)
# The treaty cue is the bare head or the ``yleis``-prefixed compound
# (``yleissopimus`` = convention) ONLY — the same narrow modifier the original
# enumeration allowed.  A wider ``-sopimus`` compound (``työsopimus`` =
# employment contract, ``kauppasopimus`` = trade agreement) is a domestic
# contract, NOT a treaty, so the modifier prefix is intentionally restricted to
# ``yleis`` to avoid those false positives.
_TREATY_WORD_RE = re.compile(
    rf"\b(?:yleis)?(?:{_SOPIMUS_HEAD_ALT})\b",
    re.IGNORECASE,
)
