...
def _pick_exemplars(rows: Tuple[OpenLawOperationAuditRow, ...]) -> dict[str, EvidenceRowSummary]:
    ...
    wanted = (
        ...
        ("metadata_lane", lambda row: row.status == "metadata_unsupported"),
        ("unplanned_document", lambda row: row.status == "planning_failed"),
    ...
    *,
    ...
    lines.extend(
        [
            ...
            "- It does not yet audit Open Law annotation metadata as legal body text.",
            "- It does not yet implement non-COMAR emergency-register expiry semantics.",
