...
# This SE per-op projector is one of the three apply-site consumers of the
...
# sibling UK/NO projectors. ``core_registry_finding_kind`` stays empty: the
# concrete emitted finding kind is dynamic and carried per-finding as
# ``core_finding_kind`` in the extra detail. SE has no declared_recovery at v0,
# so the recovery-aware fields the NO sibling threads are simply absent here.
_PROBE_SPEC = ProbeSpec(
    env_flag=_PROBE_ENV_FLAG,
    kind=SE_MUTATION_BOUNDARY_PER_OP_VIOLATION_KIND,
    skipped_kind=SE_MUTATION_BOUNDARY_PER_OP_VIOLATION_KIND.replace(
...
def project_boundary_observation(
    ...
    *,
    ...
    uses the SAME shared ``probe_adjudication`` harness spec the UK/NO sibling
    ...
    core finding's detail, so the SE record cannot drift from core / Finland / UK.
    """
