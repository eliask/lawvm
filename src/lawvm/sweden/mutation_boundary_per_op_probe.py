...
``uk_legislation/mutation_boundary_per_op_probe.py``. ``sweden/grafter.py``
...
This module wires the audit into the **SE apply fold's per-op apply loop**
(``apply_se_ops``) as an OBSERVATION-ONLY, env-gated probe. It is a consumer of
...
``Finding`` the core emits, and PROJECTS it into the SE
...
The SE apply fold has no ``strict_profile`` signaling path today (Finland's
``strict_profile``/``is_strict`` system is absent in ``apply_se_ops``). So the
probe is the discipline-disclosing first step, not the strict verdict. STRICT
ENFORCEMENT (block under a future SE strict_profile lane) is multi-session.

WHY A SNAPSHOT PROBE (post-apply, not pre-apply filter)
``audit_op_mutation_boundary`` requires before/after ``IRNode`` snapshots.
``apply_se_ops`` folds over a frozen ``IRNode`` ``body`` reassigned through the
...
* It does NOT block the op — the mutation has already landed by the time the
  probe runs. A future SE strict_profile lane can flip ``blocking=True``.
...
* It does NOT carry declared_migration / declared_recovery / declared_editorial
  prefixes at v0 — SE does not yet surface those per-op from a typed carrier.
...
* It observes the statute ``body`` tree only. ``apply_se_ops`` also mutates the
  ...
  leaves ``body`` unchanged and so reads clean here. That is the conservative
  v0 scope — the body diff is the per-op footprint the storage-boundary model
  covers; the supplements lane is a separate surface, not silently merged in.
"""
...
def boundary_probe_enabled() -> bool:
    ...
    keyed on the SE flag — one fact, read from core, default-off.
    """
...
def probe_op_mutation_boundary(
    *,
    ...
    that core finding into the SE ``CompileAdjudication`` interop surface — it
    does not re-run the verifier or re-derive the diagnostic shape, so the SE
    ...
    audit = audit_op_mutation_boundary(
        ...
        is_strict=False,  # SE fold has no strict_profile signal yet (§2.9).
        # SE IRStatute.body is the top-level tree (no hcontainer wrapper);
