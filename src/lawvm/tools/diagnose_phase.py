...
def _run_tree_detector(
    ...
    if detector in ("duplicate_label", "illegal_edge", "all_tree"):
        ...
        for w in find_text_duplication_warnings(ir):
            ...
            left = w.get("left", "?")
            right = w.get("right", "?")
            tokens = w.get("shared_token_count", 0)
        ...
        for w in find_flattened_sublist_warnings(ir):
            ...
            raw_sample = w.get("label_sample")
            sample_str = ", ".join(str(x) for x in (list(raw_sample) if isinstance(raw_sample, list) else [])[:8])
            if wkind == "flattened_sublist_interleaved":
                raw_fams = w.get("repeated_families")
                fams = ", ".join(str(x) for x in (raw_fams if isinstance(raw_fams, list) else []))
                violations.append(f"{path}: flattened {node_kind} family interleaved ({fams}) [{sample_str}]")
            elif wkind == "flattened_sublist_reset":
                dom = str(w.get("dominant_family") or "?")
                mx = str(w.get("max_before_reset") or "?")
                rst = str(w.get("reset_label") or "?")
                violations.append(f"{path}: flattened {node_kind} {dom}-family reset at {rst!r} (max was {mx}) [{sample_str}]")
            else:
...
def _matches_target(violation: str, target_path: str) -> bool:
    """True if the violation path contains the target_path as a contiguous segment.

    Violation format:  "body/chapter:4/section:20: duplicate subsection:2 ..."
    Target examples:   "chapter:4/section:20", "section:20", "chapter:4"
    """
    if not target_path:
        return True
    sep = ": "
    idx = violation.find(sep)
    if idx == -1:
        return False
    path_str = violation[:idx]             # "body/chapter:4/section:20"
