...
_EU_RULE_SPECS: Dict[str, str] = {
    ...
    "eu_fmx4_grammar_paragraph_of_article_replace_missing_payload": "A paragraph-of-article REPLACE instruction ('Paragraph (N) of Article M is replaced ...') with no quoted replacement body is a typed grammar diagnostic, not a silent empty-payload paragraph op.",
    ...
    "eu_fmx4_grammar_annex_as_set_out_target_unresolved": "A sole-annex indirect amendment ('the Annex ... as set out in the Annex to this Regulation') names no numbered base-annex coordinate, so it is a typed annex-lane extraction gap rather than an executable bare annex target.",
    ...
    "eu_fmx4_grammar_foreign_target_instruction": "An omnibus amending-act instruction whose prose names one or more instruments none of which is the base act being lowered against is a typed foreign-target skip (#221 cross-target guard), never an op — applying another instrument's amendment to this base is misapplication (convicted by the consolidation oracle on 32022R2309@20230216), while a false skip is only a visible typed coverage residual.",
    "eu_fmx4_instrument_keyword": "The bounded classifier that finds each instrument keyword (Regulation/Directive/Decision) in an amendment instruction's prose — step 1 of the two-step #221 foreign-target citation scan (backtracking-bounded via compile_classifier_regex; a one-shot pattern over the variable (EU)/(EC)/No decorations is the nested-optional-quantifier shape the regex-safety lint forbids).",
    "eu_fmx4_instrument_number": "The bounded classifier that extracts the A/B instrument number pair from the short window after an instrument keyword — step 2 of the two-step #221 foreign-target citation scan; the lookarounds reject partial matches inside a longer number so a cited '2016/44' is never read out of '2016/443'.",
    "eu_fmx4_quoted_article_heading": "The bounded classifier that recognizes a quoted whole-article replacement body's own leading 'Article N' heading; the #221 payload normalization strips it only when the captured number equals the payload's target label (in the IR the heading is the node LABEL, not text — no grafted rendering carries it in the article body).",

    # --- FMX4 amendment grammar, Increment 4 (omnibus multi-point NP lane) --------------
    "eu_fmx4_amended_as_follows": "The bounded classifier that recognizes the omnibus multi-point instruction opener ('… is amended as follows:') — the gate that switches an instruction ARTICLE from single-instruction lowering to NP sub-instruction iteration.",
    "eu_fmx4_amendment_verb": "The bounded amendment-verb census (replaced/inserted/added/deleted/repealed/amended/renumbered/substituted/shall read) behind the non-amending-provision classifier: an instruction ARTICLE whose quote-free prose carries none of these (and no for:/read: formula) is the amending act's own substantive/final provision.",
    "eu_fmx4_grammar_non_amending_provision": "An instruction ARTICLE with no amendment verb and no corrigendum formula in its quote-free prose is typed as the amending act's OWN substantive/final provision (family non_amending_provision) — it cannot touch the base act, so it is not a coverage gap; a misclassification cannot hide: an unexecuted real amendment surfaces as a billable divergence at a fully-covered oracle anchor.",
    "eu_fmx4_grammar_act_title_replace": "An act-TITLE replace ('The title of Regulation X is replaced by the following:') is a typed metadata-lane residual (family act_metadata_gap): the act title has no unit coordinate on the article-only compare surface, so the gap is recorded without poisoning article scoring.",
    "eu_fmx4_grammar_annex_indirect_instructions": "An article-level 'Annex X is amended in accordance with the Annex to this Regulation' instruction ships EMBEDDED amendment instructions inside the amender's own annex; a typed annex-lane gap (family annex_extraction_gap) — the article-only compare surface is untouched — never a whole-annex replace with instruction text as payload.",
    "eu_fmx4_grammar_annex_root_instruction_sequence": "A numberless ANNEX-root manifestation whose body is itself a base-annex amendment-instruction sequence ('The Annexes to Regulation X are amended as follows: …', the real 32012R0630 shape) is a typed annex-lane gap; a numberless annex root NOT naming the base's annexes stays a plain extraction gap (it may be the amender's own annex stored in lieu of its act — the 32014L0059 acquisition shape — whose base-article amendments are then unknowable).",
    "eu_fmx4_grammar_np_payload_unresolved": "An NP sub-instruction whose quoted payload could not be resolved (no QUOT block; per-label structural zip failed; plural target over an unsplittable block) is a typed grammar diagnostic, not a silent empty-payload op.",
    "eu_fmx4_grammar_np_annex_indirect_instructions": "An NP-level annex indirection ('Annex X is amended in accordance with / as set out in Annex Y to this Regulation') carries its amendments inside the amender's own annex Y — a typed annex-lane capability gap, never an op with instruction text as payload.",
    "eu_fmx4_grammar_np_annex_insert_payload_unresolved": "An NP annex INSERT naming a payload annex the manifestation does not carry is a typed annex-lane gap, not a silent empty-annex insert.",
    "eu_fmx4_grammar_np_nest_target_unresolved": "An NP '… is amended as follows:' nest whose target the grammar could not resolve is ONE typed residual covering the whole nest — its sub-instructions are never lowered against a guessed context (mis-targeting corrupts; a typed residual is only visible coverage loss).",
    "eu_fmx4_grammar_np_unaddressable_target": "An NP instruction targeting a title/heading/introductory wording is recognized but has no coordinate in the IR system — a typed residual, never silently dropped.",
