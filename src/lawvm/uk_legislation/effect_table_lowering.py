...
from lawvm.uk_legislation.source_parent_payloads import (
    ...
    _source_parent_prefix_with_child_text,
)
...
def try_lower_table_row_insert(
    *,
    ...
    if table_row_insert_selector is None and action == "insert":
        source_parent_table_entry_insert = _source_parent_prefix_with_child_text(
            extracted_el=extracted_el,
            source_root=source_root,
            extracted_text=extracted_text,
            instruction_pattern=_SOURCE_PARENT_TABLE_ENTRY_INSERT_RE,
    ...
    if (
        ...
        in {"column_entry", "column_final_entry", "each_column_entry", "each_column_final_entry"}
...
def try_lower_repeal_table_effect(
    *,
    ...
    if repeal_table_text_repeal.recognized and repeal_table_text_repeal.original:
        ...
        elif repeal_table_rule_id == _UK_REPEAL_TABLE_REFERENCE_TEXT_REPEAL_RULE_ID:
            reason_code = "unique_repeal_table_extent_row_reference"
            reason = (
                "UK repeal-table source row matched the affected Act and "
                "provision exactly, and its extent cell names one singular "
                "reference repeal; lowering emits a text delete for the named "
                "reference instead of replaying the broad repeal schedule."
            )
