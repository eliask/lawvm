...
def _load_affecting_instrument_xml(act_id: str, archive: Any) -> tuple[bytes, str]:
    ...
    *,
    ...
    for effect in effects:
        ...
        if act_id not in by_act:
            xml_bytes, locator = _load_affecting_instrument_xml(act_id, archive)
            date = _single_instrument_commencement_date(xml_bytes) if xml_bytes else ""
            by_act[act_id] = (date, locator)
        date, locator = by_act[act_id]
        if not date:
            continue
        overrides[effect.effect_id] = date
