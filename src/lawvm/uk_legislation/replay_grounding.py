...
class UKReplayGroundingMixin:
    ...
    def ground_ids(self):
        ...
        # PR2 (audit XJUR-02 / AGENTS.md §2.3): ``pending_cleared_events`` was
        ...
        # is unaffected by tree rebuilds. This preserves byte-identical dedup
        # semantics: matched events still suppress the cleared events queued by
        # the earlier passes for the same logical node.
        ...
        def _queue_cleared_alignment_event(
            ...
            *,
            ...
            if before_eid:
                pending_cleared_events[path_key] = _alignment_event(
                    node,
                    before_eid=before_eid,
                    after_eid=None,
                    match_method=match_method,
        ...
        def _append_alignment_event(
            ...
            *,
            ...
            pending_cleared_events.pop(path_key, None)
        ...
        def _clear_eids(
            ...
            if eid and eid not in oracle_id_values:
                # Non-canonical EID — clear it so the grounding pass can
                # assign the correct oracle ID.
                new_attrs = {
                    k: v for k, v in new_attrs.items() if k not in ("eId", "id")
                }
        ...
        def _ensure_local_eid(
            ...
            elif "eId" not in new_attrs and "id" not in new_attrs and kind_value != "body":
            ...
            if oracle_id:
                ...
                if uk_is_transparent_wrapper_kind(kind_name):
                    if "eId" in node.attrs:
                        ...
                        new_attrs = {
                            k: v for k, v in node.attrs.items() if k != "eId"
                        }
                        node = dc_replace(node, attrs=new_attrs)
                        _append_alignment_event(
                            node,
                            path_key=path_key,
                            before_eid=before_eid,
