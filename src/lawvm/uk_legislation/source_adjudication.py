...
def classify_uk_manual_compile_frontier(
    *,
    ...
    if compare_shape_norm == "text_patch_preimage_absent_from_target_surfaces":
        return {
            "status": UKManualFrontierStatus.SOURCE_INSUFFICIENT.value,
    ...
    if compare_shape_norm == "range_to_container_target_absent":
        return {
            "status": UKManualFrontierStatus.MANUAL_COMPILE_CANDIDATE.value,
    ...
    if (
        ...
        return {
            "status": UKManualFrontierStatus.DETERMINISTIC_FRONTEND_SUPPORTED.value,
    ...
    if "uk_effect_source_range_definition_entry_at_end_insert_rejected" in blocking_rules:
        return {
            "status": UKManualFrontierStatus.DETERMINISTIC_FRONTEND_CANDIDATE.value,
    ...
    if temporal_cessation_exclusion_rule:
        return {
            "status": UKManualFrontierStatus.NON_TEXTUAL_OR_OUT_OF_SCOPE.value,
    ...
    if "uk_effect_crossheading_source_target_mismatch_rejected" in blocking_rules:
        return {
            "status": UKManualFrontierStatus.SOURCE_OR_FEED_TARGET_CONFLICT.value,
    ...
    return {
        "status": UKManualFrontierStatus.UNCLASSIFIED_FRONTIER.value,
