...
from lawvm.uk_legislation.nlp_parser import (
    ...
    UK_AT_END_STEP_INSERT_RULE_ID,
    UK_BEFORE_STEP_INSERT_RULE_ID,
    ...
    *,
    ...
    if UK_EXCEPT_PHRASE_SUBSTITUTION_RULE_ID in rule_ids:
        for fragment in fragment_subs or []:
            ...
            excluded_phrase = parts[2] if len(parts) == 3 else ""
            _append_uk_effect_lowering_observation(
                lowering_rejections_out,
    ...
    if UK_AFTER_QUOTED_ANCHOR_EXCEPT_CHILD_INSERT_RULE_ID in rule_ids:
        for fragment in fragment_subs or []:
            if (
                str(fragment.get("rule_id") or "")
                != UK_AFTER_QUOTED_ANCHOR_EXCEPT_CHILD_INSERT_RULE_ID
            ...
            _append_uk_effect_lowering_observation(
                ...
                family="text_rewrite_lowering",
                reason_code="explicit_after_anchor_except_child_insert_text_patch",
                reason=(
                    "UK effect source explicitly inserts after a quoted anchor "
                    "in each place except inside a named child provision; lowering "
                    "preserves the excluded child as part of the text selector."
                ),
                effect=effect,
                extracted_el=extracted_el,
                extracted_text=extracted_text,
    ...
    if UK_AFTER_QUOTED_ANCHOR_CLOSING_QUOTE_INSERT_RULE_ID in rule_ids:
        for fragment in fragment_subs or []:
            if str(fragment.get("rule_id") or "") != UK_AFTER_QUOTED_ANCHOR_CLOSING_QUOTE_INSERT_RULE_ID:
                continue
            _append_uk_effect_lowering_observation(
                lowering_rejections_out,
                rule_id=UK_AFTER_QUOTED_ANCHOR_CLOSING_QUOTE_INSERT_RULE_ID,
                family="text_rewrite_lowering",
                reason_code="after_quoted_anchor_closing_quote_insert",
                reason=(
                    "UK effect source uses a closing quotation mark where an "
    ...
    if UK_AFTER_WORDS_IN_BRACKETS_INSERT_RULE_ID in rule_ids:
        for fragment in fragment_subs or []:
            if str(fragment.get("rule_id") or "") != UK_AFTER_WORDS_IN_BRACKETS_INSERT_RULE_ID:
                continue
            _append_uk_effect_lowering_observation(
                lowering_rejections_out,
                rule_id=UK_AFTER_WORDS_IN_BRACKETS_INSERT_RULE_ID,
                family="text_rewrite_lowering",
                reason_code="after_words_in_brackets_insert",
                reason=(
                    "UK effect source inserts words immediately after the "
    ...
    if UK_AT_END_STEP_INSERT_RULE_ID in rule_ids:
        for fragment in fragment_subs or []:
            if str(fragment.get("rule_id") or "") != UK_AT_END_STEP_INSERT_RULE_ID:
                continue
            _append_uk_effect_lowering_observation(
                lowering_rejections_out,
                rule_id=UK_AT_END_STEP_INSERT_RULE_ID,
                family="text_rewrite_lowering",
                reason_code="at_end_step_insert",
                reason=(
                    "UK effect source inserts text at the end of a named Step; "
                    "lowering preserves this as a Step-boundary selector rather "
                    "than appending to the whole target provision."
                ),
                effect=effect,
                extracted_el=extracted_el,
                extracted_text=extracted_text,
    ...
    if UK_BEFORE_STEP_INSERT_RULE_ID in rule_ids:
        for fragment in fragment_subs or []:
            if str(fragment.get("rule_id") or "") != UK_BEFORE_STEP_INSERT_RULE_ID:
                continue
            _append_uk_effect_lowering_observation(
                lowering_rejections_out,
                rule_id=UK_BEFORE_STEP_INSERT_RULE_ID,
                family="text_rewrite_lowering",
                reason_code="before_step_insert",
                reason=(
                    "UK effect source inserts text before a named Step; "
                    "lowering preserves this as a Step-boundary selector rather "
                    "than prepending to the whole target provision."
                ),
                effect=effect,
                extracted_el=extracted_el,
                extracted_text=extracted_text,
    ...
    if UK_AFTER_ANCHOR_TO_END_UNQUOTED_SUBSTITUTION_RULE_ID in rule_ids:
        for fragment in fragment_subs or []:
            if (
                str(fragment.get("rule_id") or "")
                != UK_AFTER_ANCHOR_TO_END_UNQUOTED_SUBSTITUTION_RULE_ID
            ...
            _append_uk_effect_lowering_observation(
                ...
                family="text_rewrite_lowering",
                reason_code="after_anchor_to_end_unquoted_substitution_text_patch",
                reason=(
                    "UK source text substitutes the words after a quoted "
                    "anchor with a payload that is carried without enclosing "
                    "quotes; lowering records that source-text recovery and "
                    "preserves the explicit anchor-to-end boundary as a "
                    "bounded text patch."
                ),
                effect=effect,
                extracted_el=extracted_el,
    ...
    if UK_PASSIVE_QUOTED_SUBSTITUTION_RULE_ID in rule_ids:
        for fragment in fragment_subs or []:
            if str(fragment.get("rule_id") or "") != UK_PASSIVE_QUOTED_SUBSTITUTION_RULE_ID:
                continue
            _append_uk_effect_lowering_observation(
                lowering_rejections_out,
                rule_id=UK_PASSIVE_QUOTED_SUBSTITUTION_RULE_ID,
                family="text_rewrite_lowering",
                reason_code="explicit_passive_quoted_substitution_text_patch",
                reason=(
                    "UK effect source explicitly substitutes a quoted expression "
    ...
    if UK_RANGE_TO_END_BARE_QUOTED_SUBSTITUTION_RULE_ID in rule_ids:
        _append_uk_effect_lowering_observation(
            lowering_rejections_out,
            rule_id=UK_RANGE_TO_END_BARE_QUOTED_SUBSTITUTION_RULE_ID,
            family="text_rewrite_lowering",
            reason_code="explicit_range_to_end_bare_quoted_text_patch",
            reason=(
                "UK source text uses a bare quoted preimage with 'onwards' "
                "after already identifying the local words context; lowering "
                "preserves that as a bounded TEXT_FROM_*_TO_END text patch on "
                "the effect-feed target."
            ),
            effect=effect,
            extracted_el=extracted_el,
            extracted_text=extracted_text,
    ...
    if UK_SECTION_REFERENCE_REPEAL_RULE_ID in rule_ids:
        _append_uk_effect_lowering_observation(
            lowering_rejections_out,
            rule_id=UK_SECTION_REFERENCE_REPEAL_RULE_ID,
            family="text_rewrite_lowering",
            reason_code="explicit_section_reference_repeal_text_patch",
            reason=(
                "UK source text explicitly omits a reference to a named "
                "section; lowering preserves that as a bounded text deletion "
                "inside the affected target rather than deleting an entry, "
                "paragraph, or parent container."
            ),
            effect=effect,
            extracted_el=extracted_el,
            extracted_text=extracted_text,
    ...
    if UK_AT_END_DANGLING_INSERT_QUOTE_RULE_ID in rule_ids:
        _append_uk_effect_lowering_observation(
            lowering_rejections_out,
            rule_id=UK_AT_END_DANGLING_INSERT_QUOTE_RULE_ID,
            family="text_rewrite_lowering",
            reason_code="at_end_dangling_insert_quote_text_patch",
            reason=(
                "UK source text explicitly inserts a quoted payload at the "
                "end of the affected target, but the source fragment lacks a "
