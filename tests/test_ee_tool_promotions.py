...
def test_bench_regression_guard_prints_phase_timing_deltas(
    ...
    assert "Baseline total: 10.000s" in out
    assert "Current total : 13.500s" in out
    assert "Delta         : +3.500s" in out
    assert "replay=+2.000s" in out
    assert "compile_ops=+1.500s" in out
    assert "RESULT: PASS" in out
...
def test_bench_regression_guard_can_fail_on_phase_regressions(
    ...
    out = capsys.readouterr().out
    assert rc == 1
    assert "Phase regressions > 1.000s : 1 row/phase cell(s)" in out
    assert "ukpga/2000/1" in out
    assert "replay" in out
    assert "RESULT: FAIL" in out


def test_bench_regression_guard_can_limit_phase_regressions_to_selected_phases(
    tmp_path,
    monkeypatch,
    capsys,
) -> None:
    ...
    out = capsys.readouterr().out
    assert rc == 0
    assert "Phase regressions for phase(s) replay > 1.000s : 0  (none)" in out
    assert "RESULT: PASS" in out


def test_bench_regression_guard_fails_when_selected_phase_has_no_cells(
    tmp_path,
    monkeypatch,
    capsys,
) -> None:
...
def test_bench_regression_guard_requires_phase_timing_columns_when_enabled(
    tmp_path,
    monkeypatch,
    capsys,
) -> None:
    ...
    out = capsys.readouterr().out
    assert rc == 1
    assert "ERROR: baseline and current have no common phase timing rows" in out
