...
def test_1984_719_cross_chapter_oracle_match_finds_section_107_real_corpus() -> None:
    ...
    assert oracle_key in matches
    assert matches[oracle_key]["oracle_section"] == "chapter:13/section:107"
    assert float(matches[oracle_key]["oracle_section_score"]) >= 0.95


def test_1984_719_typed_section_claims_select_cross_chapter_oracle_exact_for_107() -> None:
    master = pinned_replay("1984/719", mode="legal_pit", quiet=True)
    replay_sections = extract_ir_sections(master.materialized_state.ir)
    replay_texts = {key: render_node_text(node) for key, node in replay_sections.items()}
    ...
    rows = build_section_claims_typed(
        ...
        cross_chapter_oracle_matches=matches,
    )
    ...
    assert row["cross_chapter_oracle_match"]["oracle_section"] == "chapter:13/section:107"


def test_1984_719_typed_proof_claims_promote_cross_chapter_oracle_exact_for_107() -> None:
    master = pinned_replay("1984/719", mode="legal_pit", quiet=True)
    replay_sections = extract_ir_sections(master.materialized_state.ir)
    replay_texts = {key: render_node_text(node) for key, node in replay_sections.items()}
...
def test_2017_320_cross_chapter_replay_match_finds_section_243_real_corpus() -> None:
    master = pinned_replay("2017/320", mode="legal_pit", quiet=True)
    replay_sections = extract_ir_sections(master.materialized_state.ir)
    replay_texts = {key: render_node_text(node) for key, node in replay_sections.items()}
    ...
    assert matches[oracle_key]["replay_section"] == "part:7/chapter:1/section:243"
    assert matches[oracle_key]["runner_up_replay_section"] == "part:5/chapter:22/section:243"
...
def test_2017_320_cross_chapter_replay_match_finds_section_266_real_corpus() -> None:
    master = pinned_replay("2017/320", mode="legal_pit", quiet=True)
    replay_sections = extract_ir_sections(master.materialized_state.ir)
    replay_texts = {key: render_node_text(node) for key, node in replay_sections.items()}
    ...
    assert matches[oracle_key]["replay_section"] == "part:7/chapter:2/section:266"
    assert float(matches[oracle_key]["replay_section_score"]) >= 0.95


def test_2017_320_typed_section_claims_keep_243_as_unresolved_cross_chapter_replay_drift() -> None:
    master = pinned_replay("2017/320", mode="legal_pit", quiet=True)
    replay_sections = extract_ir_sections(master.materialized_state.ir)
    replay_texts = {key: render_node_text(node) for key, node in replay_sections.items()}
    ...
    rows = build_section_claims_typed(
        ...
        cross_chapter_replay_matches=matches,
    )
    row = rows[0].to_legacy_row()
...
def test_2017_320_typed_proof_claims_keep_243_as_unresolved() -> None:
    master = pinned_replay("2017/320", mode="legal_pit", quiet=True)
    replay_sections = extract_ir_sections(master.materialized_state.ir)
    replay_texts = {key: render_node_text(node) for key, node in replay_sections.items()}
    ...
    claim = next(
        c for c in claims
        if c["kind"] == "UNRESOLVED.address_projection.cross_chapter_replay_drift"
    )
    assert claim["tier"] == "UNRESOLVED"
    assert claim["inference_rule"] == (
        "replay_matches_same_label_section_in_different_chapter_than_oracle"
...
def test_2017_320_typed_section_claims_select_cross_chapter_replay_exact_for_266() -> None:
    master = pinned_replay("2017/320", mode="legal_pit", quiet=True)
    replay_sections = extract_ir_sections(master.materialized_state.ir)
    replay_texts = {key: render_node_text(node) for key, node in replay_sections.items()}
...
def test_2017_320_typed_proof_claims_promote_cross_chapter_replay_exact_for_266() -> None:
    master = pinned_replay("2017/320", mode="legal_pit", quiet=True)
    replay_sections = extract_ir_sections(master.materialized_state.ir)
    replay_texts = {key: render_node_text(node) for key, node in replay_sections.items()}
