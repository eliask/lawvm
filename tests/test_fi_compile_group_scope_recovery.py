...
@pytest.mark.xfail(
    ...
    reason=(
        ...
        "(feature `source_owned_body_scope_over_prior_repeal_address`) now has "
        "a real-corpus pull from 1993/1501 via 2016/773 §148, but is still "
        "contradicted by the official Finlex oracle: the real pinned_replay "
        "`test_replay_xml_1973_36_materializes_live_missing_sections` requires "
        "the live/carry-forward chapter to WIN for a reinstated section "
        "(1973/36 §27 stays in chapter 4), which makes the live-target gate "
        "load-bearing. A broad 2026-06-24 source-body override experiment fixed "
        "part of 1993/1501 but caused full-bench Levenshtein regressions. "
        "Re-enable source-body override ONLY with a signal that distinguishes "
        "it from the 1973/36 case and passes full bench. strict=True so this "
        "flips loud the moment such a discriminator lands."
    ),
)
