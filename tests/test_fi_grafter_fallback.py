...
def test_prune_container_payload_sections_drops_foreign_scoped_shadow_in_new_chapter() -> None:
    ...
    A sibling change ("Prune foreign-scoped FI container payload sections")
...
def test_enrich_ops_overrides_live_stem_host_with_adjacent_source_body_existing_edit() -> None:
    master = ReplayState(
        ir=IRNode(
            kind=IRNodeKind.BODY,
            children=(
                IRNode(
                    kind=IRNodeKind.CHAPTER,
                    label="6",
                    children=(IRNode(kind=IRNodeKind.SECTION, label="58"),),
                ),
                IRNode(
                    kind=IRNodeKind.CHAPTER,
                    label="7",
                    children=(
                        ...
                        IRNode(kind=IRNodeKind.SECTION, label="66"),
                    ),
                ),
            ),
        )
    ...
    muutos_tree = etree.fromstring(
        ...
        <act xmlns="http://docs.oasis-open.org/legaldocml/ns/akn/3.0">
          <body>
            <chapter>
              <num>7 luku</num>
              <heading>Käyttäjän oikeudet</heading>
              <section><num>58 a §</num><content><p>new section</p></content></section>
              <section><num>61 §</num><content><p>existing section replacement</p></content></section>
              <section><num>66 §</num><content><p>another existing section replacement</p></content></section>
            </chapter>
          </body>
        </act>
        """
    ...
    insert_op = AmendmentOp(
        ...
        lo=LegalOperation(
            ...
            sequence=1,
            action=StructuralAction.INSERT,
            target=LegalAddress(path=(("chapter", "6"), ("section", "58a"))),
        ),
    )
    replace_op = AmendmentOp(
        op_id="replace_61",
        ...
        lo=LegalOperation(
            ...
            sequence=2,
            action=StructuralAction.REPLACE,
            target=LegalAddress(path=(("chapter", "7"), ("section", "61"))),
        ),
    )
    second_replace_op = AmendmentOp(
        op_id="replace_66",
        op_type=OpType.REPLACE,
        target_unit_kind="section",
        target_section="66",
        ...
        lo=LegalOperation(
            ...
            sequence=3,
            action=StructuralAction.REPLACE,
            target=LegalAddress(path=(("chapter", "7"), ("section", "66"))),
        ),
    )

    got = _enrich_ops_from_amendment_tree(
        [insert_op, replace_op, second_replace_op],
        "2024/1",
        muutos_tree,
        master=master,
        source_model=AmendmentSourceModel.from_tree(muutos_tree),
    ...
    assert got[0].target_cols.target_chapter == "7"
    assert got[0].witness_rule_id == "fi_live_stem_scope_overridden_by_corroborated_source_body"
    assert got[0].lo is not None
    assert got[0].lo.target.path == (("chapter", "7"), ("section", "58a"))


def test_enrich_ops_keeps_live_stem_host_for_suffix_after_source_body_existing_edits() -> None:
    master = ReplayState(
        ir=IRNode(
            kind=IRNodeKind.BODY,
            children=(
                IRNode(
                    kind=IRNodeKind.CHAPTER,
                    label="6",
                    children=(IRNode(kind=IRNodeKind.SECTION, label="66"),),
                ),
                IRNode(
                    kind=IRNodeKind.CHAPTER,
                    label="7",
                    children=(
                        IRNode(kind=IRNodeKind.SECTION, label="61"),
                        IRNode(kind=IRNodeKind.SECTION, label="66"),
                        IRNode(kind=IRNodeKind.SECTION, label="70"),
                    ),
                ),
            ),
        )
        ...
        <act xmlns="http://docs.oasis-open.org/legaldocml/ns/akn/3.0">
          <body>
            <chapter>
              ...
              <section><num>61 §</num><content><p>existing section replacement</p></content></section>
              <section><num>66 §</num><content><p>another existing section replacement</p></content></section>
              <section><num>66 a §</num><content><p>new suffix section</p></content></section>
              <section><num>70 §</num><content><p>later existing section replacement</p></content></section>
            </chapter>
          </body>
        </act>
        """
    ...
    insert_op = AmendmentOp(
        ...
        lo=LegalOperation(
            ...
            sequence=1,
            action=StructuralAction.INSERT,
            target=LegalAddress(path=(("chapter", "6"), ("section", "66a"))),
        ),
    )
    replace_61 = AmendmentOp(
        op_id="replace_61",
        op_type=OpType.REPLACE,
        target_unit_kind="section",
        target_section="61",
    ...
    replace_70 = AmendmentOp(
        ...
        lo=LegalOperation(
            ...
            sequence=4,
            action=StructuralAction.REPLACE,
            target=LegalAddress(path=(("chapter", "7"), ("section", "70"))),
        ),
    )

    got = _enrich_ops_from_amendment_tree(
        [insert_op, replace_61, replace_66, replace_70],
        "2024/1",
        muutos_tree,
        master=master,
        source_model=AmendmentSourceModel.from_tree(muutos_tree),
    ...
    assert got[0].witness_rule_id != "fi_live_stem_scope_overridden_by_corroborated_source_body"
    assert got[0].lo is not None
    assert got[0].lo.target.path == (("chapter", "6"), ("section", "66a"))


def test_enrich_ops_keeps_live_stem_host_when_source_body_chapter_has_whole_section_repeals() -> None:
    master = ReplayState(
        ir=IRNode(
            kind=IRNodeKind.BODY,
            children=(
        ...
        <act xmlns="http://docs.oasis-open.org/legaldocml/ns/akn/3.0">
          <body>
            <chapter>
              ...
              <section><num>61 §</num><content><p>existing section replacement</p></content></section>
              <section><num>66 §</num><content><p>another existing section replacement</p></content></section>
              <section><num>70 §</num><content><p>section to repeal</p></content></section>
            </chapter>
          </body>
        </act>
        """
    ...
    got = _enrich_ops_from_amendment_tree(
        [insert_op, replace_61, replace_66, repeal_70],
        "2024/1",
        muutos_tree,
        master=master,
        source_model=AmendmentSourceModel.from_tree(muutos_tree),
    ...
    assert got[0].witness_rule_id != "fi_live_stem_scope_overridden_by_corroborated_source_body"
    assert got[0].lo is not None
    assert got[0].lo.target.path == (("chapter", "6"), ("section", "58a"))
...
def test_replay_2003_393_places_2006_759_58a_insert_in_source_body_chapter() -> None:
    compiled_ops: list[dict[str, object]] = []
    ...
    insert_matches = [
        op for op in compiled_ops
        if op.get("source_statute") == "2006/759"
        and op.get("action") == "insert"
        and op.get("target_unit_kind") == "section"
        and op.get("target_norm") == "58a"
    ]
    assert len(insert_matches) == 1
    assert insert_matches[0]["target_chapter"] == "7"
    assert insert_matches[0]["witness_rule_id"] == "fi_live_stem_scope_overridden_by_corroborated_source_body"
...
def test_official_consolidation_horizon_keeps_multi_section_future_repeal_at_cutoff() -> None:
    ...
    decision = choose_replay_horizon(
        ReplayHorizonRequest(
            mode="official_consolidation",
            as_of="",
            cutoff_date=dt.date(1995, 3, 17),
            amendment_records=[
                {
                    "statute_id": "1995/386",
                    "included": True,
                    "effective_date": dt.date(1995, 6, 1),
                    "issue_date": dt.date(1995, 3, 17),
                }
            ],
            oracle_version_amendment_id="1995/386",
            compiled_ops=[
                {
                    "source_statute": "1995/386",
                    "action": "repeal",
                }
            ],
            legal_operations=legal_operations,
    ...
    assert decision.materialize_as_of == "1995-03-17"
    assert decision.expires_as_of == "1995-03-17"
    assert decision.oracle_materialize_as_of == "1995-03-17"


def test_official_consolidation_horizon_keeps_ordinary_mixed_future_repeal_at_cutoff() -> None:
    legal_operations = [
        LegalOperation(
            op_id="repeal_section_7",
            sequence=0,
...
def test_official_consolidation_horizon_keeps_generic_section_one_chapter_future_repeal_at_cutoff() -> None:
    ...
    legal_operations = [
        LegalOperation(
            ...
            source=OperationSource(
                statute_id="2022/1386",
                effective="2023-01-01",
            ),
        ),
        LegalOperation(
            op_id="repeal_chapter_2a",
            sequence=1,
            action=StructuralAction.REPEAL,
            target=LegalAddress(path=(("chapter", "2a"),)),
            source=OperationSource(
                statute_id="2022/1386",
                effective="2023-01-01",
            ),
    ...
    decision = choose_replay_horizon(
        ReplayHorizonRequest(
            mode="official_consolidation",
            as_of="",
            cutoff_date=dt.date(2022, 12, 29),
            amendment_records=[
                {
                    "statute_id": "2022/1386",
                    "included": True,
                    "effective_date": dt.date(2023, 1, 1),
                    "issue_date": dt.date(2022, 12, 29),
                }
            ],
            oracle_version_amendment_id="2022/1386",
            compiled_ops=[
                {
                    "source_statute": "2022/1386",
                    "action": "repeal",
                }
            ],
            legal_operations=legal_operations,
...
def test_official_consolidation_horizon_uses_reflected_future_repeal_effective_date() -> None:
    ...
    decision = choose_replay_horizon(
        ReplayHorizonRequest(
            ...
            legal_operations=legal_operations,
            oracle_reflected_section_original_versions=("2022/1386",),
            oracle_single_future_repeal_overlay_versions=(),
            replay_print=lambda _message: None,
        )
    )
    ...
    assert decision.expires_as_of == "2023-01-01"
    assert decision.oracle_materialize_as_of == "2023-01-01"


def test_official_consolidation_horizon_uses_absent_future_repeal_target_effective_date() -> None:
    legal_operations = [
        LegalOperation(
    ...
    decision = choose_replay_horizon(
        ReplayHorizonRequest(
            mode="official_consolidation",
            as_of="",
            cutoff_date=dt.date(2023, 4, 14),
            amendment_records=[
                {
                    "statute_id": "2023/741",
                    "included": True,
                    "effective_date": dt.date(2026, 1, 1),
                    "issue_date": dt.date(2023, 4, 14),
                }
            ],
            oracle_version_amendment_id="2023/741",
            compiled_ops=[
                {
                    "source_statute": "2023/741",
                    "action": "repeal",
                }
            ],
            legal_operations=legal_operations,
            oracle_reflected_section_original_versions=(),
            oracle_single_future_repeal_overlay_versions=(),
            oracle_absent_repeal_target_versions=("2023/741",),
            replay_print=lambda _message: None,
        )
    )

    assert decision.materialize_as_of == "2026-01-01"
    assert decision.expires_as_of == "2026-01-01"
    assert decision.oracle_materialize_as_of == "2026-01-01"
