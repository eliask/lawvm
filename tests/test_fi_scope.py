...
def test_chapter_chunks_accept_letter_suffix_genitive_luvun_form() -> None:
    from lawvm.finland.scope import chapter_chunks_from_johtolause

    assert chapter_chunks_from_johtolause(
        "muutetaan 13 a luvun otsikko, 146 a ja 147 §, 14 luvun otsikko"
    ) == [("13a", " otsikko, 146 a ja 147 §, "), ("14", " otsikko")]


def test_chapter_chunks_do_not_scope_sections_from_prior_law_citation_tail() -> None:
    from lawvm.finland.scope import chapter_chunks_from_johtolause

    text = (
        "muutetaan 22 luvun otsikko, 209 n §:n 2 momentti sekä 209 r, 209 s ja 216 §, "
        "sellaisina kuin niistä ovat 13 a luvun otsikko laissa 1486/1994 ja "
        "22 luvun otsikko laissa 325/2003, sekä lisätään lakiin uusi 147 a §, "
        "lakiin siitä lailla 605/2009 kumotun 148 §:n tilalle uusi 148 §"
    )

    assert chapter_chunks_from_johtolause(text) == [
        ("22", " otsikko, 209 n §:n 2 momentti sekä 209 r, 209 s ja 216 §, ")
    ]


def test_johtolause_binds_sections_after_lettered_chapter_heading_target() -> None:
    johto = "muutetaan 13 a luvun otsikko, 146 a ja 147 §, 14 luvun otsikko"

    assert _johtolause_explicitly_binds_chapter_section(johto, "13a", "147") is True
    assert _johtolause_explicitly_binds_chapter_section(johto, "14", "147") is False


def test_strip_unjustified_chapter_scope_preserves_sections_after_lettered_chapter_heading() -> None:
    master = SimpleNamespace(
        ir=IRNode(
            kind=IRNodeKind.BODY,
            children=(
                IRNode(
                    kind=IRNodeKind.CHAPTER,
                    label="13a",
                    children=(IRNode(kind=IRNodeKind.SECTION, label="146a"),),
                ),
                IRNode(
                    kind=IRNodeKind.CHAPTER,
                    label="14",
    ...
    heading = LegalOperation(
        op_id="heading",
        sequence=1,
        action=StructuralAction.REPLACE,
        target=LegalAddress(path=(("chapter", "13a"),), special=FacetKind.HEADING),
    )
    section = LegalOperation(
        op_id="section",
        sequence=2,
        action=StructuralAction.REPLACE,
        target=LegalAddress(path=(("chapter", "13a"), ("section", "147"))),
    )
    ...
    assert got[1].target.path == (("chapter", "13a"), ("section", "147"))
    assert "chapter_scope_stripped_unique_section" not in got[1].provenance_tags
