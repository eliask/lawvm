"""§1.7 same-moment cross-act incompatible-payload ambiguity detection for SE.

Two affecting acts effecting the same target on the same effective date with
incompatible whole-target payloads were, before B1, resolved silently by op
order in :func:`apply_se_ops` with ZERO same-moment coverage. The pre-pass
wired into :func:`apply_se_ops` (mirroring the EE/UK precedent and reusing the
shared core detector verbatim) emits a BLOCKING finding so the silent pick is
visible and strict-rejectable. Apply order is unchanged — the finding is
additive.

Guard-liveness (AGENTS.md §2.9): drives synthesized conflicting ops through the
FULL ``apply_se_ops`` production lane, not just the detector unit.
"""
...
SE_SAME_MOMENT_AMBIGUITY_RULE_ID = (
    "se_same_moment_cross_act_incompatible_payload_ambiguous"
)


def _statute_with_section(label: str = "5", text: str = "Ursprunglig text") -> IRStatute:
    return IRStatute(
        statute_id="se/test",
        title="Test",
        body=IRNode(
...
def _same_moment_findings(
    ...
    return [a for a in adjudications if a.kind == SE_SAME_MOMENT_AMBIGUITY_RULE_ID]
...
def test_two_distinct_acts_replace_same_target_same_effective_date_emits_ambiguity() -> None:
    ...
    ops = [
        _replace_section_op(
            ...
            sequence=1,
            section_label="5",
            source_id="se/act-a/2025",
            effective="2026-01-01",
            replacement_text="Act A ny lydelse.",
        ),
        _replace_section_op(
            op_id="se-replace-B",
            sequence=2,
            section_label="5",
            source_id="se/act-b/2025",
            effective="2026-01-01",
            replacement_text="Act B ny lydelse.",
        ),
    ]
    adjudications: list[CompileAdjudication] = []
    ...
    assert detail["rule_id"] == SE_SAME_MOMENT_AMBIGUITY_RULE_ID
    ...
    assert conflicting_op_ids == {"se-replace-A", "se-replace-B"}
...
def test_repeal_versus_replace_same_moment_is_incompatible() -> None:
    ...
    ops = [
        _replace_section_op(
            ...
            source_id="se/act-a/2025",
            effective="2026-01-01",
            replacement_text="Act A ersattning.",
        ),
        _repeal_section_op(
            op_id="se-repeal-B",
            sequence=2,
...
def test_validated_precedence_claim_resolves_ambiguity() -> None:
    ...
    claim = SameMomentPrecedenceClaim(
        ...
        conflicting_affecting_acts=("se/act-a/2025", "se/act-b/2025"),
        winner_affecting_act_id="se/act-b/2025",
    ...
    assert finding["resolved_by_claim_winner_affecting_act_id"] == "se/act-b/2025"
