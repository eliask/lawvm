...
def test_uncompiled_oracle_addition_is_source_chain_lead_not_medium_suspect(
    tmp_path,
) -> None:
    report = _write_report(
        tmp_path,
        [
            {
                ...
                "n_mutation_boundary_unexplained_paths": 0,
                "source_chain_frontier_reasons": [],
                "oracle_only_eid_samples": ["section-6-1a"],
                "oracle_only_uncompiled_addition_eid_samples": ["section-6-1a"],
                "oracle_only_uncompiled_addition_change_ids": ["d30p378"],
                "agreement_residual": {
                    "owner_phase": "compare_oracle_classification",
                    "missing_proofs": [],
                    "forbidden_shortcuts": ["oracle_score_as_source_truth"],
    ...
    assert len(rows) == 1
    assert rows[0].candidate_family == "oracle_uncompiled_addition_source_chain_lead"
    assert rows[0].confidence == "source_chain_lead"
    assert rows[0].oracle_only_uncompiled_addition_samples == ("section-6-1a",)
    assert rows[0].oracle_only_uncompiled_addition_change_ids == ("d30p378",)
    assert rows[0].missing_proofs == (
