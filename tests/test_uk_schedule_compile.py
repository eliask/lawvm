...
def test_compile_source_carried_definition_entry_insert_from_parent_context() -> None:
    ...
    effect = UKEffectRecord(
        effect_id="key-source-carried-definition-entry",
        effect_type="words inserted",
        applied=True,
        requires_applied=True,
        modified="2013-04-01",
    ...
    assert (
        f"{_NOTE_TEXT_REWRITE_RULE}uk_effect_source_carried_definition_entry_insert_text_patch"
        in ops[0].provenance_tags
    )
...
def test_compile_source_carried_definition_entry_insert_rejects_non_definition_payload() -> None:
    ...
    assert extracted_el is not None
    effect = UKEffectRecord(
        effect_id="key-source-carried-definition-entry-negative",
        effect_type="words inserted",
        applied=True,
        requires_applied=True,
        modified="2013-04-01",
...
def test_compile_table_entry_deictic_column_substitution_uses_previous_source_sibling() -> None:
    ...
    extracted_el = source_root.find(f".//{{{_LEG_NS}}}P3[@id='section-24-5-a-iii']")
    assert extracted_el is not None
    effect = UKEffectRecord(
        effect_id="uk_test_table_entry_deictic_column_substitution",
        effect_type="word substituted",
        applied=True,
        requires_applied=True,
        modified="2026-03-06",
        ...
        affecting_year="2026",
        affecting_number="2",
        affecting_provisions="s. 24(5)(a)(iii)",
        affecting_title="Test Amendment Act",
        in_force_dates=[{"date": "2026-03-06", "prospective": "false"}],
    )
    lowering_records: list[dict[str, Any]] = []
    ...
    assert ops[0].target.path == (("section", "166"), ("subsection", "5"))
    assert ops[0].text_patch is not None
    assert ops[0].text_patch.selector.match_text == "two-thirds"
    assert ops[0].text_patch.replacement == "one-half"
    selector_tag = next(tag for tag in ops[0].provenance_tags if tag.startswith(_NOTE_TABLE_CELL_SELECTOR))
    selector = json.loads(selector_tag.removeprefix(_NOTE_TABLE_CELL_SELECTOR))
    assert selector["rule_id"] == "uk_effect_table_entry_deictic_label_column_text_patch"
    assert selector["selector_mode"] == "unique_entry_cell"
    assert selector["entry_label"] == "6b"
    assert selector["column_index"] == 3
    assert selector["source_context"] == "previous_source_sibling_entry_label"
    assert any(
        record["rule_id"] == "uk_effect_table_entry_deictic_label_column_text_patch"
        and record["reason_code"] == "source_parent_table_entry_paragraph_selector"
        and record["blocking"] is False
        for record in lowering_records
    )
...
def test_compile_table_entry_deictic_column_substitution_blocks_without_source_context() -> None:
    ...
    effect = UKEffectRecord(
        effect_id="uk_test_table_entry_deictic_column_without_context",
        effect_type="word substituted",
        applied=True,
        requires_applied=True,
        modified="2026-03-06",
...
def test_compile_repeal_table_quoted_words_text_repeal() -> None:
    ...
    assert extracted_el is not None
    effect = UKEffectRecord(
        effect_id="uk_test_repeal_table_quoted_words",
        effect_type="words repealed",
        applied=True,
        requires_applied=False,
        modified="2004-11-28",
    ...
    assert ops[0].text_patch is not None
    assert ops[0].text_patch.kind is TextPatchKindEnum.DELETE
    assert ops[0].text_patch.selector.match_text == "23,"
    assert (
        f"{_NOTE_TEXT_REWRITE_RULE}uk_effect_repeal_table_quoted_words_text_repeal"
        in ops[0].provenance_tags
    )
...
def test_compile_repeal_table_structural_section_range_member_repeal() -> None:
    ...
    assert extracted_el is not None
    effect = UKEffectRecord(
        effect_id="uk_test_repeal_table_structural_range_member",
        effect_type="repealed",
        applied=True,
        requires_applied=False,
        modified="2007-08-01",
        affected_uri="/id/asp/2000/6/section/27",
        affected_class="ScottishAct",
        affected_year="2000",
        affected_number="6",
        affected_provisions="s. 27",
        affecting_uri="/id/asp/2006/8",
        affecting_class="ScottishAct",
        affecting_year="2006",
        affecting_number="8",
        affecting_provisions="sch.",
        affecting_title="Test Repeal Act",
        in_force_dates=[{"date": "2007-08-01", "prospective": "false"}],
    )
    lowering_records: list[dict[str, Any]] = []
...
def test_compile_repeal_table_structural_section_range_non_member_unresolved() -> None:
    ...
    assert extracted_el is not None
    effect = UKEffectRecord(
        effect_id="uk_test_repeal_table_structural_range_non_member",
        effect_type="repealed",
        applied=True,
        requires_applied=False,
        modified="2007-08-01",
        affected_uri="/id/asp/2000/6/section/32",
        affected_class="ScottishAct",
        affected_year="2000",
        affected_number="6",
        affected_provisions="s. 32",
        affecting_uri="/id/asp/2006/8",
        affecting_class="ScottishAct",
        affecting_year="2006",
        affecting_number="8",
...
def test_compile_repeal_table_definition_entry_text_repeal() -> None:
    ...
    assert extracted_el is not None
    effect = UKEffectRecord(
        effect_id="uk_test_repeal_table_definition_entry",
        effect_type="words repealed",
        applied=True,
        requires_applied=False,
        modified="2007-08-01",
    ...
    assert ops[0].text_patch is not None
    assert ops[0].text_patch.kind is TextPatchKindEnum.DELETE
    assert ops[0].text_patch.selector.match_text == "TEXT_DEFINITION_ENTRY_the 1988 Act"
    assert (
        f"{_NOTE_TEXT_REWRITE_RULE}uk_effect_repeal_table_definition_entry_text_repeal"
        in ops[0].provenance_tags
    )
    ...
    assert any(
        ...
        and record["blocking"] is False
        and record["strict_disposition"] == "record"
        and record["extent_cell"] == "In section 58(1), the definition of “the 1988 Act”."
        and record["original"] == "TEXT_DEFINITION_ENTRY_the 1988 Act"
        for record in lowering_records
    )
...
def test_compile_repeal_table_singular_entry_for_text_repeal() -> None:
    ...
    assert extracted_el is not None
    effect = UKEffectRecord(
        effect_id="uk_test_repeal_table_singular_entry_for",
        effect_type="words repealed",
        applied=True,
        requires_applied=False,
        modified="2013-04-01",
...
def test_compile_repeal_table_definition_child_remains_unresolved() -> None:
    ...
    assert extracted_el is not None
    effect = UKEffectRecord(
        effect_id="uk_test_repeal_table_definition_child",
        effect_type="words repealed",
        applied=True,
        requires_applied=False,
        modified="2003-04-04",
        ...
        affecting_provisions="Sch. 15",
        affecting_title="Test Repeal Act",
        in_force_dates=[{"date": "2003-04-04", "prospective": "false"}],
    )
    lowering_records: list[dict[str, Any]] = []
...
def test_compile_repeal_table_quoted_words_blocks_without_unique_owned_row() -> None:
    ...
    assert extracted_el is not None
    effect = UKEffectRecord(
        effect_id="uk_test_repeal_table_no_quoted_words",
        effect_type="words repealed",
        applied=True,
        requires_applied=False,
        modified="2004-11-28",
...
def test_compile_direct_table_after_that_entry_insert_preserves_logical_row_group_payload() -> None:
    ...
    extracted_el = source_root.find(f".//{{{_LEG_NS}}}P3[@id='schedule-17-paragraph-7-b']")
    assert extracted_el is not None
    effect = UKEffectRecord(
        effect_id="uk_test_direct_table_after_that_entry_group_insert",
        effect_type="words inserted",
        applied=True,
        requires_applied=True,
        modified="2025-01-01",
        affected_uri="/id/ukpga/2020/17/section/190",
        affected_class="UnitedKingdomPublicGeneralAct",
        affected_year="2020",
        affected_number="17",
        affected_provisions="s. 190(3) Table",
        affecting_uri="/id/ukpga/2022/32",
        affecting_class="UnitedKingdomPublicGeneralAct",
        affecting_year="2022",
        affecting_number="32",
        affecting_provisions="Sch. 17 para. 7(b)",
        affecting_title="Police, Crime, Sentencing and Courts Act 2022",
        in_force_dates=[{"date": "2025-01-01", "prospective": "false"}],
    )
    lowering_records: list[dict[str, Any]] = []
                    ...
                    children=(
                        IRNode(
                            ...
                            children=(
                                IRNode(
                                    kind=IRNodeKind.ROW,
                                    children=(IRNode(kind=IRNodeKind.CELL, text="drug rehabilitation requirement"),),
                                ),
                                IRNode(
                                    kind=IRNodeKind.ROW,
...
def test_replay_table_entry_logical_group_insert_after_rowspanned_anchor_group() -> None:
    selector = {
        ...
        "column_index": 1,
        "entry_index": 1,
        "relating_text": "electronic monitoring requirement",
    ...
    op = LegalOperation(
        op_id="uk_test_table_relating_entry_group_insert",
        sequence=1,
        action=StructuralAction.INSERT,
        target=LegalAddress(path=(("section", "190"), ("subsection", "3"))),
        payload=IRNode(
            kind=IRNodeKind.TABLE,
            children=(
                IRNode(
                    ...
                    children=(
                        IRNode(
                            kind=IRNodeKind.CELL,
                            text="An electronic whereabouts monitoring requirement",
                            attrs={"rowspan": "2"},
                        ),
                        IRNode(kind=IRNodeKind.CELL, text="Responsible person"),
                    ),
                ),
                IRNode(
                    kind=IRNodeKind.ROW,
                    children=(IRNode(kind=IRNodeKind.CELL, text="Consent person"),),
                ),
            ),
        ),
        provenance_tags=(f"{_NOTE_TABLE_ROW_INSERT_SELECTOR}{json.dumps(selector)}",),
                                    ...
                                    children=(
                                        IRNode(
                                            ...
                                            children=(
                                                IRNode(
                                                    kind=IRNodeKind.CELL,
                                                    text="An electronic monitoring requirement",
                                                    attrs={"rowspan": "2"},
                                                ),
                                                IRNode(kind=IRNodeKind.CELL, text="Responsible old person"),
                                            ),
                                        ),
                                        IRNode(
                                            kind=IRNodeKind.ROW,
                                            children=(IRNode(kind=IRNodeKind.CELL, text="Consent old person"),),
                                        ),
                                        IRNode(
                                            kind=IRNodeKind.ROW,
                                            children=(IRNode(kind=IRNodeKind.CELL, text="Drug rehabilitation requirement"),),
...
def test_compile_direct_table_relating_entry_instruction_uses_owned_selector() -> None:
    ...
    assert len(ops) == 1
    assert ops[0].action is StructuralAction.INSERT
    assert ops[0].target.path == (("section", "201"),)
    assert ops[0].payload is not None
    assert ops[0].payload.kind is IRNodeKind.ROW
    assert [child.text for child in ops[0].payload.children] == [
        "electronic whereabouts monitoring requirement"
