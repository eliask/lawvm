...
def test_validate_semantic_claim_rejects_duplicate_live_target_precondition_id() -> None:
    target_text_hash = hashlib.sha256(b"table text").hexdigest()
    claim = _claim_row()
    proposed_outcome = claim["proposed_outcome"]
    assert isinstance(proposed_outcome, dict)
...
def test_validate_semantic_claim_rejects_occurrence_count_if_any_source_preview_mismatches() -> None:
    claim = _claim_row(source_preview='for "old" substitute "new"')
    claim["source_preview"] = 'for "old" substitute "new"; for "old" omit'
    proposed_outcome = claim["proposed_outcome"]
    assert isinstance(proposed_outcome, dict)
    proposed_outcome["source_text_preconditions"] = [
        {
    ...
    assert (
        'source_text_preconditions[1].occurrence_count 1 does not match supplied '
        'source text counts [1, 2] for \'"old"\''
    ) in row["validation_issues"]
    assert (
        'source_text_preconditions[1].max_occurrences 1 is below supplied source '
        'text counts [1, 2] for \'"old"\''
    ) in row["validation_issues"]
    assert row["replay_authorized"] is False
...
def test_validate_semantic_claim_accepts_source_text_precondition_order() -> None:
    claim = _claim_row(source_preview="after the entry relating to X insert the row")
    proposed_outcome = claim["proposed_outcome"]
    assert isinstance(proposed_outcome, dict)
    proposed_outcome["source_text_preconditions"] = [
...
def test_validate_semantic_claim_rejects_source_text_precondition_reversed_order() -> None:
    claim = _claim_row(source_preview="insert the row after the entry relating to X")
    proposed_outcome = claim["proposed_outcome"]
    assert isinstance(proposed_outcome, dict)
    proposed_outcome["source_text_preconditions"] = [
        {
    ...
    assert (
        ...
        "'source-anchor' is not satisfied by supplied source text indexes [1]"
    ) in row["validation_issues"]
    assert row["replay_authorized"] is False


def test_validate_semantic_claim_rejects_source_text_precondition_unknown_order_ref() -> None:
    claim = _claim_row(source_preview="after the entry relating to X insert the row")
    proposed_outcome = claim["proposed_outcome"]
    assert isinstance(proposed_outcome, dict)
    proposed_outcome["source_text_preconditions"] = [
    ...
    assert row["validator_status"] == "rejected_source_text_mismatch"
    assert (
        "source_text_preconditions[1].after_precondition_ids references unknown "
        "source text precondition 'missing-anchor'"
    ) in row["validation_issues"]
    assert row["replay_authorized"] is False


def test_validate_semantic_claim_rejects_duplicate_source_text_order_refs() -> None:
    claim = _claim_row(source_preview="after the entry relating to X insert the row")
    proposed_outcome = claim["proposed_outcome"]
    assert isinstance(proposed_outcome, dict)
    proposed_outcome["source_text_preconditions"] = [
    ...
    assert row["validator_status"] == "rejected_source_text_mismatch"
    assert (
        "source_text_preconditions[2].after_precondition_ids[2] duplicates "
        "after_precondition_ids[1] 'source-anchor'"
    ) in row["validation_issues"]
    assert row["replay_authorized"] is False


def test_validate_semantic_claim_rejects_source_text_precondition_order_without_common_preview() -> None:
    claim = _claim_row(source_preview="entry relating to X")
    claim["source_preview"] = "insert the row"
    proposed_outcome = claim["proposed_outcome"]
    assert isinstance(proposed_outcome, dict)
    proposed_outcome["source_text_preconditions"] = [
        {
    ...
    assert row["validator_status"] == "rejected_source_text_mismatch"
    assert (
        "source_text_preconditions[2].after_precondition_ids cannot be checked "
        "because 'insert the row' and 'entry relating to X' do not occur in the "
        "same supplied source text"
    ) in row["validation_issues"]
    assert row["replay_authorized"] is False
...
def test_validate_semantic_claim_rejects_duplicate_source_text_precondition_id() -> None:
    ...
    assert row["validator_status"] == "rejected_source_text_mismatch"
    assert (
        "source_text_preconditions[2].precondition_id duplicates "
        "source_text_preconditions[1].precondition_id 'source-anchor'"
    ) in row["validation_issues"]
    assert row["replay_authorized"] is False
...
def test_validate_semantic_claim_rejects_duplicate_family_specific_proof_refs() -> None:
    claim = _claim_row(source_preview="after the entry relating to X insert the row")
    proposed_outcome = claim["proposed_outcome"]
    assert isinstance(proposed_outcome, dict)
    proposed_outcome["source_text_preconditions"] = [
    ...
    assert row["validator_status"] == "rejected_schema"
    assert (
        "operation_family_proofs[1].entry_ownership_ids[2] duplicates "
        "entry_ownership_ids[1] 'entry-carrier'"
    ) in row["validation_issues"]
    assert row["replay_authorized"] is False
...
def test_validate_semantic_claim_rejects_duplicate_validator_check_ids() -> None:
    ...
    assert row["validator_status"] == "rejected_schema"
    assert (
        "proposed_outcome.validator_checks[2].check_id duplicates "
        "proposed_outcome.validator_checks[1].check_id "
        "'claim_identifies_exact_table_carrier'"
    ) in row["validation_issues"]
    assert row["replay_authorized"] is False
...
def test_validate_semantic_claim_rejects_duplicate_live_ids_in_operation_family_proof() -> None:
    claim = _claim_row(source_preview="after the entry relating to X insert the row")
    proposed_outcome = claim["proposed_outcome"]
    assert isinstance(proposed_outcome, dict)
    proposed_outcome["source_text_preconditions"] = [
...
def test_validate_semantic_claim_rejects_appropriate_place_unreferenced_anchor_live_path() -> None:
    parent_text_hash = hashlib.sha256(b"parent text").hexdigest()
    anchor_text_hash = hashlib.sha256(b"anchor text").hexdigest()
    claim = _claim_row(
        source_preview='at the appropriate place insert "new listed entry"',
    ...
    assert (
        ...
        "anchor_live_target_precondition_paths references live path "
        "'section:1/subsection:1' outside proof live_target_precondition_ids or "
        "live_target_precondition_paths"
    ) in row["validation_issues"]
    assert row["replay_authorized"] is False
...
def test_validate_semantic_claim_rejects_duplicate_ownership_claim_ids() -> None:
    claim = _claim_row()
    claim["ownership_claims"] = [
        {
            "ownership_id": "source_named_table_surface",
    ...
    assert row["validator_status"] == "rejected_schema"
    assert (
        "ownership_claims[2].ownership_id duplicates "
        "ownership_claims[1].ownership_id 'source_named_table_surface'"
    ) in row["validation_issues"]
    assert row["replay_authorized"] is False
